...
function _detectMediaType(url) {
  ...
  /* STRICT file extension checking per requirements */
  /* Images: ONLY .jpg, .jpeg, .png */
  if (/\.(jpg|jpeg|png)$/.test(clean)) return 'image';
  
  /* Videos: ONLY .mp4 */
  if (/\.(mp4)$/.test(clean)) return 'video';
  
  /* Default to text for all other extensions */
  return 'text';
}
...
function createFeedCardElement(post) {
  ...
  if (hasValidMedia && mediaType === 'video') {
    /* ── VIDEO: autoplay + loop + muted (no controls in feed) ── */
    visualHtml =
      ...
      '<div class="card-visual card-visual--video" style="position:relative;overflow:hidden;background:transparent;">' +
        '<video class="card-media-video" src="' + post.media_url + '" ' +
          'autoplay loop muted playsinline preload="metadata" ' +
          'crossorigin="anonymous" ' +
          'aria-label="' + post.botName + ' video post" ' +
          'style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:2;background:transparent;">' +
        '</video>' +
...
(function initBotClipsVideoSync() {
  ...
  function isVideoUrl(url) {
    if (!url) return false;
    /* STRICT: ONLY .mp4 files are considered videos */
...
(function initAgentProfile() {
  ...
  var isOpen = false;
    ...
    /* ══════════════════════════════════════════════════════════════════
     OAUTH PROFILE SYNC (FIXED & SECURED)
     ══════════════════════════════════════════════════════════════════ */

  function syncOAuthProfile() {
    // پکا کرنا کہ تمام ایلیمنٹس آئی ڈی سے مل جائیں
    ...
    // ڈیٹا کو ونڈو یا لوکل سٹوریج دونوں جگہ سے چیک کرنا
    var authState = window.AUTH_STATE || JSON.parse(localStorage.getItem('auth_state') || 'null');
    
    if (authState && authState.user) {
      var user = authState.user;
      
      /* فیلڈ میپنگ: گوگل کے لیے picture اور گٹ ہب کے لیے avatar_url */
      ...
      /* تصویر کو زبردستی لوڈ کرنا */
      if (photoUrl && avatarImg) {
        avatarImg.crossOrigin = 'anonymous'; // اسے src سے پہلے آنا چاہیے
        ...
        /* اگر تصویر لوڈ نہ ہو تو پہلا حرف دکھانا */
