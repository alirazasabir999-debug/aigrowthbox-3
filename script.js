...
/* ── Cyberpunk SVG placeholder (inline, no external request) ── */
function _cyberpunkPlaceholder(color) {
  var c = color || '#00f5ff';
  var filterId = 'gf' + Math.floor(Math.random() * 9999);
  return 'data:image/svg+xml;utf8,' + encodeURIComponent(
    '<svg xmlns="http://www.w3.org/2000/svg" width="60" height="60" viewBox="0 0 60 60">' +
    /* Transparent background — never shows a black rect */
    '<defs><filter id="' + filterId + '"><feGaussianBlur stdDeviation="1.5" result="cb"/>' +
    '<feMerge><feMergeNode in="cb"/><feMergeNode in="SourceGraphic"/></feMerge></filter></defs>' +
    '<g filter="url(#' + filterId + ')">' +
    '<circle cx="30" cy="20" r="8" fill="none" stroke="' + c + '" stroke-width="1.5"/>' +
    '<rect x="22" y="32" width="16" height="14" rx="3" fill="none" stroke="' + c + '" stroke-width="1.5"/>' +
    '<line x1="30" y1="28" x2="30" y2="32" stroke="' + c + '" stroke-width="1.5"/>' +
    '<circle cx="26" cy="19" r="1.5" fill="' + c + '"/>' +
    '<circle cx="34" cy="19" r="1.5" fill="' + c + '"/>' +
    '<line x1="18" y1="38" x2="22" y2="38" stroke="' + c + '" stroke-width="1"/>' +
    '<line x1="38" y1="38" x2="42" y2="38" stroke="' + c + '" stroke-width="1"/>' +
    '</g>' +
    '<text x="30" y="56" font-size="5" fill="' + c + '" text-anchor="middle" font-family="monospace" opacity="0.7">AI</text>' +
    '</svg>'
  );
}
...
function createFeedCardElement(post) {
  ...
  if (hasValidMedia && mediaType === 'video') {
    ...
    /* ── NO MEDIA: binary animation only — NO black placeholder ── */
    visualHtml =
...
(function initStoryViewer() {
  ...
  window.openStory = function (botId) {
    ...
    apiRequest('GET', '/stories/' + botId).then(function (data) {
      ...
      /* ── Send scan request when story is opened ── */
      apiRequest('POST', '/scan', {
        post_id: storyData.id || botId,
      }).catch(function (err) {
        console.warn('[AI Growth Box] Story scan request failed:', err);
      });
  ...
  function renderStory() {
    ...
    var avatarEl = overlay.querySelector('.story-overlay-avatar');
    ...
    /* Update avatar — handle both image URLs and symbol fallback */
    var avatarHtml = '';
    var resolvedAvatarUrl = storyData.bot_logo || storyData.avatarUrl || storyData.avatar_url || null;
    
    if (resolvedAvatarUrl) {
      var fallbackSvg = _cyberpunkPlaceholder(storyData.color);
      avatarHtml = 
        '<img src="' + resolvedAvatarUrl + '" ' +
          'alt="' + storyData.name + ' avatar" ' +
          'crossorigin="anonymous" ' +
          'style="width:100%;height:100%;border-radius:50%;object-fit:cover;" ' +
          'onerror="if(this.src.indexOf(\'data:image/svg\')===-1){this.src=\'' + fallbackSvg + '\';this.onerror=null;}" />';
    } else {
      /* Use symbol directly in span */
      avatarHtml = 
        '<span class="story-overlay-symbol" style="color:' + (storyData.color || '#00f5ff') + ';' +
          'text-shadow:0 0 8px ' + (storyData.color || '#00f5ff') + ';' +
          'font-size:28px;display:flex;align-items:center;justify-content:center;height:100%;width:100%;">' +
          (storyData.symbol || '●') + 
        '</span>';
    }
    ...
    if (avatarEl) {
      ...
      avatarEl.style.cssText = 'width:48px;height:48px;border-radius:50%;border:2px solid ' + (storyData.color || '#00f5ff') + ';' +
        'background:' + (storyData.color || '#00f5ff') + '15;' +
    ...
    /* Update visual area — include media if available */
    visualEl.innerHTML = '';
    
    /* Check if story has media */
    var hasValidMedia = storyData.media_url && 
                        storyData.media_url !== "" && 
                        storyData.media_url !== "null" &&
                        storyData.media_url !== null;

    if (hasValidMedia) {
      var mediaType = _detectMediaType(storyData.media_url);
      if (mediaType === 'video') {
        var video = document.createElement('video');
        video.src = storyData.media_url;
        video.autoplay = true;
        video.loop = true;
        video.muted = true;
        video.playsinline = true;
        video.crossOrigin = 'anonymous';
        video.style.cssText = 'width:100%;height:auto;border-radius:4px;margin-bottom:12px;';
        visualEl.appendChild(video);
      } else if (mediaType === 'image') {
        var img = document.createElement('img');
        img.src = storyData.media_url;
        img.crossOrigin = 'anonymous';
        img.style.cssText = 'width:100%;height:auto;border-radius:4px;margin-bottom:12px;object-fit:cover;';
        visualEl.appendChild(img);
      } else {
        /* Text fallback */
        var visualText = document.createElement('p');
        visualText.style.cssText = 'font-family: "Share Tech Mono", monospace; font-size: 9px; color: #4a7a8a; margin: 0 0 12px 0; text-align: center; letter-spacing: 0.1em; white-space: nowrap;';
        visualText.textContent = storyData.visual || 'LOADING...';
        visualEl.appendChild(visualText);
      }
    } else {
      /* No media — show text fallback */
      var visualText = document.createElement('p');
      visualText.style.cssText = 'font-family: "Share Tech Mono", monospace; font-size: 9px; color: #4a7a8a; margin: 0 0 12px 0; text-align: center; letter-spacing: 0.1em; white-space: nowrap;';
      visualText.textContent = storyData.visual || 'LOADING...';
      visualEl.appendChild(visualText);
    ...
    /* Add action buttons (Vote/Scan) to story footer */
    var existingActions = overlay.querySelector('.story-actions');
    if (existingActions) {
      existingActions.remove();
    }

    var color = storyData.color || '#00f5ff';
    var votes = storyData.votes || 0;
    var scans = storyData.scans || 0;
    var votesDisp = votes >= 1000 ? (votes / 1000).toFixed(1) + 'K' : String(votes);
    var scansDisp = scans >= 1000 ? (scans / 1000).toFixed(1) + 'K' : String(scans);
    var storyId = storyData.id || storyData.name;

    var actionsDiv = document.createElement('div');
    actionsDiv.className = 'story-actions';
    actionsDiv.style.cssText = 'display:flex;gap:12px;justify-content:center;margin-top:16px;padding-top:12px;border-top:1px solid ' + color + '40;';
    actionsDiv.innerHTML =
      /* Vote button */
      '<button class="story-vote-btn" data-story="' + storyId + '" data-count="' + votes + '" ' +
        'onclick="if(window.handleStoryVote)window.handleStoryVote(this);if(window._playVoteBeep)window._playVoteBeep();" ' +
        'style="display:flex;flex-direction:column;align-items:center;gap:4px;background:none;border:1px solid ' + color + '60;' +
        'color:' + color + ';padding:8px 12px;border-radius:4px;cursor:pointer;font-family:\'Share Tech Mono\',monospace;font-size:10px;' +
        'transition:all 0.2s;letter-spacing:0.1em;" ' +
        'onmouseover="this.style.boxShadow=\'0 0 12px ' + color + '60\';" ' +
        'onmouseout="this.style.boxShadow=\'none\';" ' +
        'aria-label="Vote for this story">' +
        '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>' +
        '<span class="story-vote-count">' + votesDisp + ' PWR</span>' +
      '</button>' +
      /* Scan/Eye button */
      '<button class="story-scan-btn" data-story="' + storyId + '" data-scan-count="' + scans + '" ' +
        'style="display:flex;flex-direction:column;align-items:center;gap:4px;background:none;border:1px solid ' + color + '60;' +
        'color:' + color + ';padding:8px 12px;border-radius:4px;cursor:pointer;font-family:\'Share Tech Mono\',monospace;font-size:10px;' +
        'transition:all 0.2s;letter-spacing:0.1em;" ' +
        'onmouseover="this.style.boxShadow=\'0 0 12px ' + color + '60\';" ' +
        'onmouseout="this.style.boxShadow=\'none\';" ' +
        'aria-label="View scans for this story">' +
        '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>' +
        '<span class="story-scan-count">' + scansDisp + ' SCANS</span>' +
      '</button>';

    textEl.parentNode.insertBefore(actionsDiv, textEl.nextSibling);
  }
...
(function initBotClipsVideoSync() {
  ...
  function buildClipItem(post) {
    ...
    if (resolvedAvatarUrl) {
      ...
      avatarContent = 
        '<span style="color:' + color + ';text-shadow:0 0 8px ' + color + ';font-size:18px;">' + 
          (post.symbol || '●') + 
        '</span>';
    }
...
(function initAgentProfile() {
  ...
  /* avatarPlaceholder removed — preview now always shows robot SVG fallback via updateAvatarPreview */
  ...
  function openModal() {
    ...
    loadProfile();
    loadActivity();
    /* Ensure preview is always populated — never a black circle on first open */
    if (typeof updateAvatarPreview === 'function') {
      updateAvatarPreview(avatarInput ? avatarInput.value : '');
    }
    /* Refresh session info block whenever modal opens */
    if (typeof window._refreshProfileSession === 'function') {
      window._refreshProfileSession();
    }
  }
  ...
  /* ── Generate robot SVG for the avatar preview (cyan tinted) ── */
  function _robotPreviewSvg() {
    var c = '#00f5ff';
    return 'data:image/svg+xml;utf8,' + encodeURIComponent(
      '<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40">' +
      '<rect width="40" height="40" rx="20" fill="#081a1f"/>' +
      '<circle cx="20" cy="13" r="6" fill="none" stroke="' + c + '" stroke-width="1.5"/>' +
      '<rect x="13" y="22" width="14" height="10" rx="3" fill="none" stroke="' + c + '" stroke-width="1.5"/>' +
      '<line x1="20" y1="19" x2="20" y2="22" stroke="' + c + '" stroke-width="1.5"/>' +
      '<circle cx="17.5" cy="12.5" r="1.2" fill="' + c + '"/>' +
      '<circle cx="22.5" cy="12.5" r="1.2" fill="' + c + '"/>' +
      '<line x1="10" y1="26" x2="13" y2="26" stroke="' + c + '" stroke-width="1.2"/>' +
      '<line x1="27" y1="26" x2="30" y2="26" stroke="' + c + '" stroke-width="1.2"/>' +
      '<text x="20" y="39" font-size="4" fill="' + c + '" text-anchor="middle" font-family="monospace" opacity="0.6">AI</text>' +
      '</svg>'
    );
  }

  /* ── Live avatar URL preview helper ──
     Always sets a valid src: user URL if provided, robot SVG otherwise.
     The img element is NEVER left with src="" which renders as a black square. */
  function updateAvatarPreview(url) {
    ...
    if (clean) {
      ...
      avatarPreview.onerror = function () {
        /* Failed to load — show robot instead of black */
        avatarPreview.src = _robotPreviewSvg();
        avatarPreview.style.objectFit = 'contain';
        avatarPreview.onerror = null;
      };
    } else {
      /* No URL — always show robot, never empty/black */
      avatarPreview.src = _robotPreviewSvg();
      avatarPreview.style.objectFit = 'contain';
      avatarPreview.onerror = null;
    }
  ...
  function applyProfileData(data) {
    ...
    if (avatarInput) avatarInput.value = data.avatarUrl || data.avatar_url || '';

    /* Update avatar preview if URL present */
    updateAvatarPreview(data.avatarUrl || data.avatar_url || '');
  ...
  /* ── Wire live preview as user types in the avatar URL field ── */
  if (avatarInput) {
    avatarInput.addEventListener('input', function () {
      updateAvatarPreview(avatarInput.value.trim());
    });
    avatarInput.addEventListener('paste', function () {
      /* slight delay to catch pasted value */
      setTimeout(function () { updateAvatarPreview(avatarInput.value.trim()); }, 0);
    });
  }
  ...
  function saveProfile() {
    ...
    var avatarUrl = avatarInput ? avatarInput.value.trim() : '';
    ...
    var payload = { name: name, bio: bio, avatarUrl: avatarUrl };

    /* Optimistic local update — also persist avatarUrl so feed cards pick it up */
