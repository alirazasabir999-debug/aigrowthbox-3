...
/* ──────────────────────────────────────────────────────────────────
   ...
   • Shows the login overlay on first visit (no session in storage)
   • Routes all auth through the central apiRequest() helper
   • Persists a lightweight session token in localStorage
   • Exposes window.AUTH_STATE for all other modules to read
   ────────────────────────────────────────────────────────────────── */
(function initAuthSystem() {

  /* ── Central auth state — readable by all other feature modules ── */
  window.AUTH_STATE = {
    loggedIn   : false,
  ...
  /* Persist + apply a logged-in session */
  function applySession(user, spectator) {
    ...
    /* Propagate to Profile module if it loaded before auth resolved */
    if (typeof window._onAuthReady === 'function') {
      window._onAuthReady(window.AUTH_STATE);
    }
  }

  /* Clear session and show the login screen */
  function logout() {
  ...
  function restoreSession() {
    try {
      ...
      applySession(data.user, data.spectator);

      /* Confirm session is still valid server-side (silent, non-blocking) */
      apiRequest('GET', '/auth/session').then(function (resp) {
        if (!resp || !resp.user) {
          /* Server says session expired — force re-login */
          logout();
        }
      });
      return true;
  ...
  /* ── OAuth helpers (redirect-based flow) ── */
  function oauthRedirect(provider) {
    setStatus('REDIRECTING_TO_' + provider.toUpperCase() + ' //', 'loading');
    /* For real OAuth, redirect to the provider URL returned by the API.
       Here we call the API first to get the redirect URL, then navigate. */
    apiRequest('POST', '/auth/' + provider, {}).then(function (data) {
      if (data && data.redirect_url) {
        window.location.href = data.redirect_url;
      } else {
        /* Fallback: let the server handle redirect directly */
        window.location.href = API_ENDPOINT + '/auth/' + provider;
      }
    });
  }
  ...
  function submitEmailLogin() {
    ...
    if (!email || !pass) {
      setStatus('ERR: CREDENTIALS_REQUIRED //', 'error');
      return;
    }

    setStatus('AUTHENTICATING //', 'loading');

    apiRequest('POST', '/auth/email', { email: email, password: pass })
      .then(function (data) {
        if (!data) {
        ...
        if (data.error) {
          setStatus('ERR: ' + (data.error || 'AUTH_FAILED') + ' //', 'error');
          return;
        }
        applySession(data.user || { name: email.split('@')[0], email: email }, false);
      });
  }
  ...
  if (btnSubmit) btnSubmit.addEventListener('click', submitEmailLogin);
  ...
  if (backdrop) {
    backdrop.addEventListener('click', function () {
      /* Only allow dismissal if already spectating or logged in */
  ...
  /* ── Init: show login if no stored session ── */
