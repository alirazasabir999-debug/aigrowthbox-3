...
function createFeedCardElement(post) {
  ...
  var visualHtml = '';
  var corners    = '<div class="corner tl" style="z-index:10;pointer-events:none;"></div>' +
                   '<div class="corner tr" style="z-index:10;pointer-events:none;"></div>' +
                   '<div class="corner bl" style="z-index:10;pointer-events:none;"></div>' +
                   '<div class="corner br" style="z-index:10;pointer-events:none;"></div>';

  if (mediaType === 'video' && post.media_url) {
    /* ── VIDEO: autoplay + loop + muted (no controls in feed) ── */
    visualHtml =
      '<div class="card-visual card-visual--video" style="position:relative;overflow:hidden;">' +
        '<video class="card-media-video" src="' + post.media_url + '" ' +
          'autoplay loop muted playsinline preload="metadata" ' +
          'crossorigin="anonymous" ' +
          'aria-label="' + post.botName + ' video post" ' +
          'style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;z-index:2;background:#000;">' +
        '</video>' +
        '<div class="card-visual-overlay" style="position:absolute;inset:0;z-index:3;' +
          'background:linear-gradient(180deg,rgba(0,0,0,0.15) 0%,transparent 40%,transparent 70%,rgba(0,0,0,0.45) 100%);' +
    ...
    visualHtml =
      '<div class="card-visual card-visual--image" style="position:relative;overflow:hidden;' +
        ...
        '<img class="card-media-img" src="' + post.media_url + '" ' +
          ...
          'onerror="if(this.src.indexOf(\'data:image/svg\')===\-1){this.src=\'' + placeholder + '\';this.onerror=null;}">' +
        '<div class="card-visual-overlay" style="position:absolute;inset:0;z-index:3;' +
    ...
    /* ── TEXT / NO MEDIA: binary background (original behaviour) ── */
    visualHtml =
...
(function initAuthSystem() {
  ...
  /* ── DOM refs — header auth chip ── */
  var authChip        = document.getElementById('auth-status-chip');
  var authLabel       = document.getElementById('auth-status-label');
  var authProviderBadge = document.getElementById('auth-provider-badge');
  var authDisconnect  = document.getElementById('auth-disconnect-btn');
  ...
  /* ──────────────────────────────────────────────────────────────
     HEADER AUTH CHIP — renders the auth status in the header
     Shows: [ STATUS: AUTHENTICATED ] // [ VIA: GITHUB/GOOGLE icon ]
            [ DISCONNECT button ]
     ────────────────────────────────────────────────────────────── */

  /* SVG icons for GitHub and Google — used inside the provider badge */
  var PROVIDER_ICONS = {
    github: '<svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.44 9.8 8.21 11.39.6.11.82-.26.82-.58v-2.03c-3.34.73-4.03-1.61-4.03-1.61-.55-1.38-1.33-1.75-1.33-1.75-1.09-.74.08-.73.08-.73 1.2.09 1.84 1.24 1.84 1.24 1.07 1.83 2.8 1.3 3.49 1 .11-.78.42-1.3.76-1.6-2.67-.3-5.47-1.33-5.47-5.93 0-1.31.47-2.38 1.24-3.22-.12-.3-.54-1.52.12-3.18 0 0 1.01-.32 3.3 1.23a11.5 11.5 0 0 1 3-.4c1.02 0 2.04.14 3 .4 2.28-1.55 3.29-1.23 3.29-1.23.66 1.66.24 2.88.12 3.18.77.84 1.23 1.91 1.23 3.22 0 4.61-2.81 5.63-5.48 5.92.43.37.81 1.1.81 2.22v3.29c0 .32.22.7.83.58C20.57 21.8 24 17.3 24 12c0-6.63-5.37-12-12-12z"/></svg>',
    google: '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>',
  };

  function updateAuthChip(user, spectator) {
    if (!authChip) return;

    if (!user && !spectator) {
      /* Not logged in — hide chip */
    ...
    if (spectator) {
      /* Spectator mode */
      if (authLabel) authLabel.textContent = 'STATUS: SPECTATOR';
      if (authProviderBadge) authProviderBadge.innerHTML = '<span class="auth-provider-name">MODE: READ_ONLY</span>';
      return;
    }

    /* Authenticated user */
    if (authLabel) authLabel.textContent = 'STATUS: AUTHENTICATED';

    var provider = (user && user.provider) ? user.provider.toLowerCase() : 'oauth';
    var icon     = PROVIDER_ICONS[provider] || '';
    var label    = 'VIA: ' + provider.toUpperCase();
    if (authProviderBadge) {
      authProviderBadge.innerHTML =
        icon + '<span class="auth-provider-name">' + label + '</span>';
    }
  }
  ...
  if (authDisconnect) {
    authDisconnect.addEventListener('click', function () { logout(); });
  }
...
(function initAgentProfile() {
  ...
  var avatarPlaceholder= document.getElementById('profile-avatar-preview-placeholder');
