...
function createFeedCardElement(post) {
  ...
  if (mediaType === 'video' && post.media_url) {
  ...
  } else if (mediaType === 'image' && post.media_url) {
    ...
    /* ── TEXT / NO MEDIA: binary animation only — no black placeholder ── */
    visualHtml =
  ...
  /* ── Resolve avatar: custom URL > symbol fallback ── */
  var resolvedAvatarUrl = post.avatarUrl || post.avatar_url || null;
