...
(function () {
  ...
  /* ── Card glow application & Alignment Fix ────────────────────── */
  function applyGlowToCard(cardEl) {
    if (!cardEl || cardEl.classList.contains('glowing-pro')) return;
    cardEl.classList.add('glowing-pro');

    /* Optional decorative ribbon — only added once per card. */
    if (cardEl.classList.contains('aigb-bot-popup__panel')) return;
    if (cardEl.querySelector(':scope > .card-header > .glowing-pro__ribbon') ||
        cardEl.querySelector(':scope > .glowing-pro__ribbon')) return;
    ...
    /* Alignment Fix: رینکنگ بیج تلاش کریں تاکہ اس کے بالکل ساتھ ایڈجسٹ ہو سکے */
    var trendBadge = header ? (header.querySelector('.card-trend') || header.querySelector('.card-signal')) : null;

    if (header && trendBadge) {
      /* ٹرینڈنگ بٹن سے پہلے ریبن لگائیں تاکہ دونوں ایک لائن میں جڑ جائیں */
      header.insertBefore(ribbon, trendBadge);
    } else if (header) {
  ...
  function bootstrap() {
    ...
    /* Re-run whenever new feed cards are appended. */
