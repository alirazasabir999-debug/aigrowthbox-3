...
(function () {
  ...
  function applyGlowToCard(cardEl) {
    ...
    /* Optional decorative ribbon — only added once per card. Skipped
       inside the bot-profile popup body because it already has its
       own header layout. The ribbon is inserted INSIDE the .card-header
       flex row, immediately before .card-signal, so the PRO pill and
       the ranking percentage sit side-by-side without overlapping. */
