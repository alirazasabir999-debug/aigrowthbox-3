/* ════════════════════════════════════════════════════════════════════
   USER STORE & MARKETPLACE LOGIC (SEPARATE FILE)
   ════════════════════════════════════════════════════════════════════ */

// یوزر کا عارضی ڈیٹا (جب آپ اسے اصلی ڈیٹا بیس سے جوڑیں گے تو یہ وہاں سے آئے گا)
...
/* 1. ماڈل (Pop-up) بنانے اور کھولنے کا لاجک */
...
function buildUserStoreModal() {
  ...
  document.getElementById('us-buy-pro-btn').onclick = function() {
      ...
      activateUserProfilePro(); // یہ فنکشن ڈبے والا PRO بیج لگائے گا
      closeUserStoreModal();
  };
...
function openUserStoreModal() {
  ...
  if (USER_MARKET_STATE.plan === 'pro') {
    adBtn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 12 20 22 4 22 4 12"></polyline><rect x="2" y="7" width="20" height="5"></rect><line x1="12" y1="22" x2="12" y2="7"></line></svg> CLAIM FREE';
...
/* 2. یوزر کی مین پروفائل پر [PRO] کا ڈبہ لگانے کا فنکشن */
function activateUserProfilePro() {
    USER_MARKET_STATE.plan = 'pro';
    var proBadge = document.getElementById('main-profile-pro-badge');
    if (proBadge) {
        proBadge.style.display = 'inline-block'; // یہ ڈبہ شو کر دے گا
    }
}

/* 3. پاور اپس کنٹرولر اور پروگریس لائن اپڈیٹ کرنے کا فنکشن */
function updateUserPowerUpTracker() {
    ...
    // اگر پرسنٹیج 0 سے کم ہو جائے تو اسے 0 پر روک دیں
...
// جب فائل لوڈ ہو تو ایک بار ٹریکر کو اپڈیٹ کر دے
